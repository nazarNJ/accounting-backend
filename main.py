...
''')
# تحديث آمن في حال كان الجدول موجوداً مسبقاً بدون عمود الحالة
try:
...
@app.get("/invoices/")
def get_invoices():
    c.execute("SELECT id, supplier_id, invoice_number, date, net_amount, vat_rate, vat_amount, gross_amount, status FROM invoices")
    return [{"id": r[0], "supplier_id": r[1], "invoice_number": r[2], "date": r[3], "net_amount": r[4], "vat_rate": r[5], "vat_amount": r[6], "gross_amount": r[7], "status": r[8]} for r in c.fetchall()]

@app.post("/invoices/")
def create_invoice(inv: InvoiceCreate):
    vat_amount = inv.net_amount * inv.vat_rate
    gross_amount = inv.net_amount + vat_amount
    c.execute("INSERT INTO invoices (supplier_id, invoice_number, date, net_amount, vat_rate, vat_amount, gross_amount, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
              (inv.supplier_id, inv.invoice_number, inv.date, inv.net_amount, inv.vat_rate, vat_amount, gross_amount, inv.status))
    conn.commit()
    return {"message": "Invoice created successfully"}
...
def update_invoice(invoice_id: int, inv: InvoiceUpdate):
    ...
    gross_amount = inv.net_amount + vat_amount
    c.execute("""
        UPDATE invoices SET supplier_id=?, invoice_number=?, date=?, net_amount=?, vat_rate=?, vat_amount=?, gross_amount=?, status=?
        WHERE id=?
    """, (inv.supplier_id, inv.invoice_number, inv.date, inv.net_amount, inv.vat_rate, vat_amount, gross_amount, inv.status, invoice_id))
    conn.commit()
...
def create_invoice_from_inventory(data: InvoiceFromInventoryCreate):
    ...
    c.execute("""
        INSERT INTO invoices (supplier_id, invoice_number, date, net_amount, vat_rate, vat_amount, gross_amount, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, (data.supplier_id, data.invoice_number, data.date, net_amount, data.vat_rate, vat_amount, gross_amount, data.status))
...
@app.get("/invoices/{invoice_id}/erechnung-xml")
def export_erechnung_xml(invoice_id: int):
    c.execute("SELECT id, supplier_id, invoice_number, date, net_amount, vat_rate, vat_amount, gross_amount, status FROM invoices WHERE id=?", (invoice_id,))
    row = c.fetchone()
    if not row:
        return {"error": "Invoice not found"}
